				...
				{cart && cart?.length > 0 ? (
					<>
						<div className="flex flex-col w-full items-start justify-start gap-3 h-[65%] overflow-scroll scrollbar-none px-4">
							{cart &&
								cart?.length > 0 &&
								cart?.map((item, i) => <CartItemCard key={i} index={i} data={item} />)}
						</div>

						<div className="bg-zinc-800 rounded-t-[60px] w-full h-[30%] flex flex-col items-center justify-center px-4 py-4 mb-6 gap-10">
					...
					<>
						<div className=" bg-red-400">
							<img src={EmptyCart} alt="empty cart" className=" bg-white" />
						</div>
					</>
				)}
			</div>
...
export const CartItemCard = ({ index, data }) => {
	...
	const decrementCart = (productId) => {
		updateItemQuantity(user?.user_id, productId, "decrement").then((data) => {
			getAllCartItems(user?.user_id).then((items) => dispatch(setCartItems(items)));
		});
	};
